...
| Nested workers | Do not append an identical stock base twice; append a different model's stock base when needed. |
| Main local execution | The opt-in `boss_main_decision_layer` feature hides root shell, stdin, apply-patch, and code-mode execution tools. Spawned Workers are excluded from this root-only boundary. |
...
the root code-mode escape hatch, and nested hybrid-base duplication. Desktop and
CLI profile assembly still require live smoke checks after rebuilding the patched
binary; external connector permissions are intentionally outside this local
